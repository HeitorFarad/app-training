...
export default function LoginProfissionalScreen() {
  ...
  const handleLogin = async () => {
    ...
    const profissional = lista.find((p) => p.email === email && p.senha === senha);

    if (profissional) {
      await AsyncStorage.setItem('@profissional_logado', JSON.stringify(profissional));
      navigation.navigate('MenuProfissional');
    } else {
